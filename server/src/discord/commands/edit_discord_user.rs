...
use super::shared::{default_data, OptionType, edit_action};
...
pub async fn edit_discord_user(ctx: &Context, interaction: &Interaction) -> Result<(), Error> {
    ...
    let user = get_options!(interaction.data.options, USER, User)?.id.0.to_string();

    query!("UPDATE Nation SET ownerDiscord = ? WHERE nationId = ?", user, nation.nationId).execute(db()).await?;
    
    nation_change(nation.id(), ChangeType::OwnerDiscord, Some(nation.ownerDiscord), Some(user), true).await?;
