...
pub async fn clear(){
    ...
    let pause_duration = Duration::from_secs(
        LongDuration::days(1).num_seconds() as u64
    );
...
async fn delete(week_ago: DateTime<Utc>) -> Result<i32, Error>{
    let count = query!("SELECT COUNT(*) as count FROM User WHERE lastVisit < ?", week_ago).fetch_one(db()).await?.count;
    
    query!("DELETE FROM User WHERE lastVisit < ?", week_ago).execute(db()).await?;
