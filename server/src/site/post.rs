...
use crate::{error::Error, database::{nation_all, db, validate_flag, add_flag, Id}};
...
pub async fn edit_nation(
    ...
    let user = query!("SELECT discord, isAdmin FROM User WHERE userId = ?", user.0).fetch_one(db()).await?;
    let nation = nation_all(edit.id, user.isAdmin).await?;

    if user.discord.as_ref() != Some(&nation.core.ownerDiscord) && !user.isAdmin{
        return Err(Error::InvalidPermissions("This nation does not belong to you".to_string()))
    }

    if let Some(flag) = &mut edit.flag{
        if flag.len() != 0{
            let file_name = flag.raw_name().ok_or(Error::JsonError("No file name".to_string()))?.dangerous_unsafe_unsanitized_raw().to_string();
            
            let extension = validate_flag(&file_name, flag.len())?;
            
            let writer = |path| async move{
                flag.persist_to(path).await
            };
            
            add_flag(nation.id(), &nation.core.name, extension, writer, user.isAdmin).await?;
        }
    }
    ...
    for edit_social in edit_socials{
        match edit_social.socials_id{
            None => {
                query!("INSERT INTO Social (platform, link, nationId) VALUES (?, ?, ?)", edit_social.platform, edit_social.link, nation.core.nationId).execute(db()).await?;
            },
            Some(id) => {
                let original = nation.socials.iter().find(|social| social.socialsId == id).ok_or(Error::SocialNotFound(edit_social.platform.clone()))?;
                
                if edit_social.link != original.link || edit_social.platform != original.platform{
                    query!("UPDATE Social SET platform = ?, link = ? WHERE socialsId = ?", edit_social.platform, edit_social.link, id).execute(db()).await?;
                }
    ...
    for delete_social in nation.socials.iter().filter(|social| !included_socials.contains(&social.socialsId)){
        query!("DELETE FROM Social WHERE socialsId = ?", delete_social.socialsId).execute(db()).await?;
    }

    if user.isAdmin{
        if let (Some(removed), Some(discord), Some(continent)) = (&edit.removed, edit.discord.as_ref(), edit.continent.as_ref()){
            if !CONTINENTS.contains(&continent.as_str()){
                return Err(Error::JsonError(format!("invalid continent: {}", continent)));
            }
            ...
            let removed_changed = |change_type| {nation_change(nation.id(), change_type, Some(nation.core.removed.to_string()), Some(removed.to_string()), user.isAdmin)};
            ...
            return Err(Error::JsonError("missing fields required for an admin submission".to_string()));
        }
    }
