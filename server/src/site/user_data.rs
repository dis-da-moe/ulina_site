use rocket::{
    http::{Status, Cookie, CookieJar, SameSite},
    request::Outcome,
    request::FromRequest,
    Request
};
...
impl<'r> FromRequest<'r> for UserId {
    ...
    async fn from_request(req: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        Outcome::Success(
        if let Some(cookie) = get_user(req.cookies())
        {
...
impl<'r> FromRequest<'r> for AdminUser {
    ...
    async fn from_request(req: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        if query!("SELECT isAdmin FROM User WHERE userId = ?", id.0).fetch_one(db()).await.unwrap().isAdmin{
...
async fn valid_id(id: i64) -> bool{
    query!("SELECT userId FROM User WHERE userId = ?", id).fetch_one(db()).await.is_ok()
}

async fn add_user<'r>(cookies: &CookieJar<'r>) -> UserId{
    let id = query!("INSERT INTO User (isAdmin) VALUES (false) RETURNING userId").fetch_one(db()).await.unwrap().userId;
